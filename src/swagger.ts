...
import { mergeRouteInfo, detectConflicts } from './auto-generate/merger';
 ...
 */
function parseJSDocRoutes(spec: any): Array<Partial<RouteInfo> & Pick<RouteInfo, 'routePath' | 'method'>> {
  if (!spec.paths) {
    return [];
  }

  const routes: Array<Partial<RouteInfo> & Pick<RouteInfo, 'routePath' | 'method'>> = [];
  ...
  for (const [path, pathItem] of Object.entries(spec.paths as Record<string, any>)) {
    ...
    for (const methodLower of methods) {
      ...
      const route: Partial<RouteInfo> & Pick<RouteInfo, 'routePath' | 'method'> = {
        routePath: path,
        method,
...
function mergeAutoGeneratedRoutes(
  ...
  const conflicts = detectConflicts(autoGenRoutes, jsdocRoutes as RouteInfo[]);
